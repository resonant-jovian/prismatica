...
pub fn generate(project_root: &Path) {
    ...
    if !data_dir.exists() {
        eprintln!("Warning: data/d3/ does not exist -- run `cargo xtask fetch d3` first");
        return;
    }

    println!("Generating d3...");

    let src_dir = project_root.join("src").join("d3");
    fs::create_dir_all(&src_dir).expect("create src/d3/");
    ...
    for entry in &json_files {
        ...
        // Check if a palette CSV exists (for Tableau10)
        let palette_path = data_dir.join(format!("{stem}_palette.csv"));
        let has_palette = palette_path.exists();
        ...
        if has_palette {
            ...
            let map = ColormapData { meta, lut };
            let code = codegen::generate_colormap_rs(&map, &const_name);
            let rs_path = src_dir.join(format!("{mod_name}.rs"));
            fs::write(&rs_path, code).expect("write .rs");

            colormap_entries.push((map.meta.name.clone(), mod_name, const_name));
        }
    }
    ...
    println!(
        "  Generated d3/mod.rs ({} maps, {} palettes)",
